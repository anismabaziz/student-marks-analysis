...
class UltimateDeliberationParser:
    ...
    def __init__(self, pdf_path: str, debug: bool = False):
        ...
        # Module detection patterns - comprehensive list
        self.module_patterns = [
            # Systèmes d'Information - text shows "d'information Systèmes" in cells
            (r'd.?information', ('SI', "Systèmes d'Information")),  # Catches "d'information" or "d information"
            (r'information.*systèmes?', ('SI', "Systèmes d'Information")),
            (r'systèmes?.*information', ('SI', "Systèmes d'Information")),
            
            # Théorie des Graphes
            (r'théorie.*graphes?', ('TG', 'Théorie des Graphes')),
            (r'graphes?.*théorie', ('TG', 'Théorie des Graphes')),
            
            # Algorithmique et Structures de Données
            (r'algorithmique.*structures.*données', ('ASD', 'Algorithmique et Structures de Données')),
            (r'structures.*données', ('ASD', 'Algorithmique et Structures de Données')),
            (r'données.*asd', ('ASD', 'Algorithmique et Structures de Données')),
            
            # Architecture des Ordinateurs
            (r'architecture.*ordinateurs?', ('AO', 'Architecture des Ordinateurs')),
            (r'ordinateurs?.*architecture', ('AO', 'Architecture des Ordinateurs')),
            
            # Logique Mathématique
            (r'logique.*mathématique', ('LM', 'Logique Mathématique')),
            
            # Méthodes Numériques
            (r'méthodes?.*numériques?', ('MN', 'Méthodes Numériques')),
            (r'numériques?', ('MN', 'Méthodes Numériques')),
            
            # Langue Étrangère
            (r'langue.*(?:étrangère|etrangere)', ('LANG', 'Langue Étrangère')),
            (r'étrangère', ('LANG', 'Langue Étrangère')),
        ]
    ...
    def _detect_modules(self):
        ...
        if self.debug:
            ...
            for i in range(min(16, len(name_row))):
                if name_row[i]:
                    print(f"  Col {i:2d}: UE={ue_row[i] if i < len(ue_row) else ''} | Name={str(name_row[i])[:60]}")
        ...
        for col_idx in range(2, len(ue_row)):
            ...
            if ue_cell.startswith('C00'):
                current_ue = ue_cell
            
            # Get module name
            name_cell = str(name_row[col_idx] or "").strip()
            
            # Skip moyenne/crédit columns
            name_lower = name_cell.lower()
            if 'moyenne' in name_lower or 'crédit' in name_lower:
                continue
            ...
            clean_name = ' '.join(name_cell.split())
            
            if self.debug:
                print(f"\n[DEBUG] Checking column {col_idx}: '{clean_name[:60]}'")
            
            # Try to match against known patterns
            module_info = None
            for pattern, (code, full_name) in self.module_patterns:
                if re.search(pattern, clean_name, re.IGNORECASE):
                    module_info = (code, full_name, current_ue)
                    if self.debug:
                        print(f"  ✓ Matched pattern '{pattern}' → {full_name}")
                    break
            
            if module_info:
                code, full_name, ue = module_info
                if full_name not in module_map:
                    module_map[full_name] = {'code': code, 'ue': ue, 'cols': []}
