...
def find_tables():
    ...
    response = supabase.table("analysis_tables").select("*").execute()
...
def set_table_valid(table_id):
    ...
    supabase.table("analysis_tables").update({"valid": True}).eq(
        "id", table_id
    ).execute()
...
def set_table_invalid(table_id):
    ...
    supabase.table("analysis_tables").update({"valid": False}).eq(
        "id", table_id
    ).execute()
...
def find_valid_tables():
    response = supabase.table("analysis_tables").select("*").eq("valid", True).execute()
